...
const tree = <Step onClick={onClick} title={title} number={number} checked={false} />;
...
test('should not display number if step is already checked', async () => {
  const { getByText, queryByText } = render(
    <Step onClick={onClick} title={title} number={number} checked={true} />
  );
...
test('should not call onClick function when step is not checked', async () => {
  const { getByText } = render(
    <Step onClick={onClick} title={title} number={number} checked={false} />
  );
